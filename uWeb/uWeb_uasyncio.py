...
class uWeb_uasyncio:
    ...
    def start(self, log=True):
        ...
        loop.create_task(
            uasyncio.start_server(self.router, self.address, self.port)
        )  # Schedule server loop
        print(
            "uWeb server started! Connect to http://%s:%s/"
            % (network.WLAN(network.STA_IF).ifconfig()[0], self.port)
        )
        loop.run_forever()
            ...
            if bool(self.request):  # check if request not empty
                ...
                if len(self.routes_dict) == 0:
                    ...
                    if (
                        ...
                        self.request_path,
                    ) in self.routes_dict.keys():
                        # check for valid route
                        loop = uasyncio.get_event_loop()
                        loop.create_task(
                            self.routes_dict[
                                (self.request_command, self.request_path)
                            ]()
                        )
                        ...
                        await self.render(
                            "404.html", layout=None, status=self.NOT_FOUND
                        )
                else:
